...
from models import vgg, resnet, VGG, myresnet
...
def quantize_model(args, criterion, data_loader, data_loader_test):
    ...
    saved_model = args.model_path + args.model + '.pth'
    if args.dataset == 'imagenet':
        ...
        if args.model == 'resnet18':
            ...
            model = vgg.VGG('vgg8', 10)
        elif args.model == 'vgg19':
            model = vgg.VGG('vgg19', 10)
        elif args.model == 'vgg16':
            model = vgg.VGG('vgg16', 10)
        elif args.model == 'vgg11':
            model = vgg.VGG('vgg11', 10)

    # Try to load user trained model
    if args.dataset != 'imagenet':
        print(f"\nLoading pretrained model {saved_model}...")
        state_dict = torch.load(saved_model)
        state_dict = state_dict['net'] ##Additional line for VGG11,VGG13, VGG16, VGG19. Comment out for VGG8
        #state_dict = remap_state_dict_keys(state_dict) ##Additional line for VGG8. Comment out for other models
        #state_dict = fill_missing_biases(model, state_dict)##Additional line for VGG8. Comment out for other models
...
def remap_state_dict_keys(state_dict):
    ...
    for k, v in state_dict.items():
        if k.startswith("features."):
            new_k = k.replace("features", "layers")
            new_state_dict[new_k] = v
        elif k.startswith("classifier.") and "weight" in k or "bias" in k:
            new_state_dict[k] = v  # Adjust if your model uses a different name
    return new_state_dict
    
def fill_missing_biases(model, state_dict):
    for name, param in model.named_parameters():
        if 'bias' in name and name not in state_dict:
            print(f"Filling missing bias: {name}")
